...
export function registerUserTools(server: McpServer, client: UploadPostMcpClient): void {
  ...
  server.registerTool(
    "list_users",
    {
      title: "List profiles",
      description:
        "List all Upload-Post profiles in the account, with their connected social accounts. The TikTok account object carries a `capabilities` array (music, location, cover_image, draft, privacy_levels) telling which optional TikTok fields that connection accepts.",
      inputSchema: {},
      outputSchema: genericResultOutputSchema,
      annotations: {
        title: "List profiles",
